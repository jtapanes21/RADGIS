...
def plot_gannt(tdf, group_column, location_column, min_column="min", max_column="max"):
    tdf = tdf.copy()

    tdf.sort_values([min_column, max_column], inplace=True)
    tdf.reset_index(drop=True, inplace=True)
    tdf["Task"] = tdf[group_column]
    tdf["Start"] = tdf[min_column]
    tdf["Finish"] = tdf[max_column]

    tdf[location_column] = tdf[location_column].astype(str)

    fig = ff.create_gantt(tdf, colors=None, index_col=location_column, reverse_colors=True, show_colorbar=True,group_tasks=True)
