...
# Used to replace string unique idents with integers i.e., 1,2,3...
def add_int_group(dataframe, uid_column):
    dataframe = dataframe.sort_by_uid_and_datetime()
    dataframe = dataframe.rename(columns = {uid_column: "original_uid"})
    dataframe[uid_column] = dataframe.groupby("original_uid", sort=False).grouper.group_info[0] + 1
    constants.UID = "original_uid"
    return dataframe
...
def column_order(dataframe,*args):
    first_cols = [*args]
    last_cols = [col for col in dataframe.columns if col not in first_cols]
    dataframe = dataframe[first_cols+last_cols]
